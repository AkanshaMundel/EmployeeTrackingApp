...
const RejectedComponent = () => {
  ...
  const fetchDataFromAPI = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/leave/rejected`);
      const data = response.data;
      console.log('dataconfrim', data);

      if (data.success && data.allRejectedLeave) {
        setLeaveRequests(data.allRejectedLeave);
      }
    } catch (error) {
        ...
        <View key={item._id} style={styles.card}>
          <View style={{flexDirection: 'column'}}>
            ...
            <Text style={{...styles.textmsg, marginTop: 12}}>
              {' '}
              Rejection Message: {item.rejectedReason}
            </Text>
            <Text style={{...styles.textmsg, marginTop: 12}}>
              {' '}
              ...
              <View style={{justifyContent: 'flex-start'}}>
                ...
                >
                  <Feather
                    ...
                    style={{marginRight: 4}}
                  />
                  <Text style={{fontSize: 12, color: '#8A2626'}}>Rejected</Text>
                </View>
              </View>
